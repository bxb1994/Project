...
router.use('/addInfo', function(req, res, next) {
    ...
    md5.update(pass);
    pass=md5.digest('hex');
    mysql.query(`insert into user (name,pass,phone,en) values ('${name}','${pass}','${phone}','${en}')`,function(error){
...
router.use('/updateInfo', function(req, res, next) {
    ...
    md5.update(pass);
    pass=md5.digest('hex');
    mysql.query(`update user set name='${name}',pass='${pass}',phone='${phone}', where id=${id}`,function(error){
...
router.use('/addNews', function(req, res, next) {
    ...
    mysql.query(`insert into news (title,phone,email,address) values ('${title}','${phone}','${email}','${address}')`,function(error){
...
router.use('/updatenewsInfo', function(req, res, next) {
    var title=req.body.title;
    var phone=req.body.phone;
    var email=req.body.email;
    var address=req.body.address;
    var id=req.body.id;
    mysql.query(`update news set title='${title}',phone='${phone}',email='${email}',address='${address}' where id=${id}`,function(error){
